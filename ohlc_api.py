...
def mexi(ticker, limit, timeframe):
    try:
        ...
        response = requests.get(
            f"https://api.mexc.co/api/v3/klines?symbol={ticker}USDT&interval={timeframe}m&limit={limit}"
        )
...
def kucoin_data(
    symbol: str, num_timeframe: int, format_timeframe: str, past_time: int = 100
):
    ...
    data = requests.get(
        f"https://api.kucoin.com/api/v1/market/candles?type={num_timeframe}{format_timeframe}&symbol={ticker}&startAt={int(start)}"
    ).json()
...
def forex(ticker, timeframe):
    ...
    # data.loc[:, "Time"] = [
    #     i.astimezone(pytz.timezone("Asia/Tehran")) for i in data["Time"]
    # ]

    data.index = data["Time"]

    data.index, data["Time"] = data["Time"].dt.tz_convert("Asia/Tehran"), data[
        "Time"
    ].dt.tz_convert("Asia/Tehran")
